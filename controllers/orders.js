...
import { createOrder, getOrderById, getOrderWithProductById } from "../models/orders.js"
...
orderController.get("/order_confirmation", (request, response) => {
    if (request.query.id) {
        getOrderWithProductById(request.query.id)
            .then(([orders_with_products]) => {
                if (orders_with_products.length > 0) {
                    let order_with_product = orders_with_products[0]
                    response.render("order_confirmation.ejs", {order_with_product: order_with_product})
                }
            })
    }
})
